/*******************************************************************************************************
  File: LiDAR_MotorControlIntegration.ino
  Description: 
  ...
  Version:
      1.0.0 - First motor and obstacle detection prototype test
  ...
  Changes:
     v1.0.0 (From motor control and LiDAR control code integrated)
...
float degreeDistances[360]; // One distance for each degree
unsigned long degreeTimestamp[360]; // Timestamp for each measurement (prevents stale readings)
...
void setup() {
  ...
  // Precompute sine and cosine values for each degree.
  for (int d = 0; d < 360; d++) {
...
void loop() {
  ...
  // Read the receiver channels and map values
...
bool arrayCmp(const char* array1, const char* array2, int len) {
  for (int i = 0; i < len; i++) {
