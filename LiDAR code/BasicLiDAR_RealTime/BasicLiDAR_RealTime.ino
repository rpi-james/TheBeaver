/*******************************************************************************************************
  ...
  Version:
    0.2.0 - Full‑circle distance buffering
  ...
  Changes:
    v0.2.0 
      ...
      - Added noDataCount logic: after 5 misses, input buffer is flushed to resync  
...
void loop() {
  ...
  if (result == 0) {
    ...
    for (int i = 0; i < 40; i++) {
      ...
      // Optionally, ignore 0 values. You might also use a method to combine (e.g., minimum or average) if several measurements fall on the same degree.
      if (d != 0) {
        // Update the distance for that degree.
  ...
  // Output one measurement per degree for a full circle.
  for (int deg = 0; deg < 360; deg++) {
    // Only output if a measurement is available.
    if (degreeDistances[deg] > 0) {
      MSGSERIAL.print("Angle: ");
      MSGSERIAL.print(deg);
      MSGSERIAL.print(" | Distance: ");
      MSGSERIAL.println(degreeDistances[deg], 2);
    }
  }
  //MSGSERIAL.println("-------------------------------");
...
int Scanner() {
  ...
  // Verify sync bytes based on your protocol (this check might need fine-tuning).
  ...
  // Determine the angular separation (delta) between successive measurements.
  float dAngle = 360.0 / 40.0;  // default incremental angle
  if (lastAngle >= 0) {
    // Compute delta between the current start and previous start (wrapped around 360)
    float delta = fmod((360.0 + startAngle - lastAngle), 360.0);
    // Use the delta only if it seems plausible (otherwise fall back to constant increments).
  ...
  for (int i = 0; i < 40; i++) {
    // Compute the angle for this measurement. Using modulo to keep within [0,360).
    packetAngles[i] = fmod((startAngle + dAngle * i), 360.0);
    // Extract the raw 16-bit distance from the two bytes.
    uint16_t rawDistance = (((unsigned char)rawdata[4 + 2 * i + 1]) << 8) |
                           ((unsigned char)rawdata[4 + 2 * i]);
    // For now, we assume the raw value is the distance. You may need scaling.
